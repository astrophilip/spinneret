...
def cleanup_neighborhoods_list():
    nyc_list = list(neighborhoods['NYC'])
    nyc_list = [n.split(":")[1] for n in nyc_list]
    nyc_list = [n.replace("_"," ") for n in nyc_list]
    nyc_list[91] = "Murray Hill (Manhattan)"
    nyc_list[156] = "Murray Hill (Queens)"

    bos_list = list(neighborhoods['BOS'])
    bos_list = [b.replace("_"," ") for b in bos_list]
    return bos_list,nyc_list

bos_list,nyc_list = cleanup_neighborhoods_list()
clean_neighborhoods = {}
clean_neighborhoods['BOS'] = bos_list
clean_neighborhoods['NYC'] = nyc_list

neighborhoods['JOINED'] = neighborhoods['BOS']+neighborhoods['NYC']
clean_neighborhoods['JOINED'] = bos_list+nyc_list
...
counts['JOINED'] = counts['BOS'] + counts['NYC']
